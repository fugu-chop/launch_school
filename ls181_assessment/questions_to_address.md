...
Database design refers to the process of structuring tables and data into different entities to represent different types of data and defining the relationships between those entities. An entity can be regarded as a set or data (or a real world object) that we want to model in our database (this does not necessarily mean an entity will be represented in only a single table).
...
- Conceptual: This is a high level database diagram, which is primarily focused on larger objects and concepts. It does not concern itself with specifics of the implementation, such as modality, cardinality and the data types within an entity).
...
- Physical: This is the most granular level of database diagram - it is concerned with the database level implementation of a conceptional model. It includes all the attributes of an entity, the data types of those attributes and rules about how those entities and attributes relate to each other (e.g. defining the modality and cardinality of relationships between models).

### What are keys? Why would we use them?
Keys are a type of constraint that are used to establish relationships _between_ entities in a database, and uniqueness of a record _within_ an entity. They are one way of identifying specific rows within a table (or related table), and can be used to ensure referential integrity (i.e. ensuring that a record in a table that is intended to reference a record in another table does so, and that these relationships are consistent). In the context of PostgresQL, this ensures that all values in a foreign key column in a given table correspond to a value in the primary key column in another table.

### What is a primary key?
A primary key is a constraint that can be applied to a table that allows references to specific rows within that table. They are unique identifiers for rows, and act as a way to establish referential integrity between two tables, in that a foreign key in a table should reference an entry in another table with the associated primary key. As part of applying a primary key constraint, this means that all values within the primary key column are intended to be unique, and not `NULL`.

### What is a foreign key?
A foreign key is a constraint that can be applied to a column within a table such that a given row can be associated with another row in a different table. Foreign keys must be non-`null`, but there is no requirement that a foreign key be unique (otherwise creating one-to-many relationships would not be possible between tables).

### What is a natural key?
A natural key is an identifier for individual records within a table that is created by using existing data within a table (e.g. through using a specific column, or combination of columns). Natural keys aren't particularly prevalent, since as they rely on context specific values, are vulnerable to external changes (e.g. product SKUs used as a natural key might be carried over between different product versions, or phone numbers can change over time). 

### What is a surrogate key?
A surrogate key is an identifier created specifically for the purposes of identifying unique rows within a table. It does not rely on existing data or it's context - the value of the key should not be affected by external circumstances.

### What is a sequence? Why would we use it? How do we access it?
A sequence is a relation that generates an incrementing value. Sequences can be used to generate surrogate keys for tables for use as primary keys. A sequence can only return a number once, regardless of whether it's used. Sequences should be used in conjunction with other constraints - sequences themselves do not prevent duplicate values from being manually inserted into a column; if a duplicate value was entered as a primary key, this would defeat the purpose of generating a unique identifier.

### What is referential integrity?
Referential integrity is an assurance that all rows in a given table that are intended to reference another row in another relation, do so in a consistent fashion. It's a way to ensure that relationships between entities are implemented correctly (data integrity).
...
### What is modality?
Modality is a term used to describe the minimum number of objects on each side of the relationship between two entities. When describing modality, we typically classify the relationship as either requiring at least 0 or 1 objects. In the case of 0 objects, we do not require a record to exist on both sides of the relationship; in the case of 1 object, this indicates that one side of the relationship requires an object before a relationship can be established, and the data inserted into the table.
...
2. One-to-many: This indicates that between two tables, one table is expected to have multiple rows in another table that can be referenced to one row.

### What are joins? What are the different types of joins?
`JOIN` clauses in SQL allow us to retrieve data from multiple relations, by linking multiple tables together through keys between tables. The types of joins are:
...
Other types of indexes include _multi-column_ indexes, which are indexes created from a combination of columns (though only certain index types support multi-column indexes, and the number of columns that can be combined in an index is limited). Another type of index is the _partial_ index, which is an index applied to certain values within a column. They can be defined using the same syntax as creating a normal index, with the addition of a `WHERE` clause. 
...
The `EXPLAIN` command also provides a start-up and total-cost, which are arbitrary values created by the SQL engine using various parameters, and seek to measure the estimated amount of effort and resources required to execute a query. A number of rows and an average width of rows in bytes.

The `EXPLAIN ANALYZE` command provides the same output as the `EXPLAIN` command, with the addition of actually executing the query, and providing the actual required to execute the query and the number of rows returned by each node. `EXPLAIN ANALYZE` is useful when extra data may be needed to compare different queries as an effort to optimise database queries. 
