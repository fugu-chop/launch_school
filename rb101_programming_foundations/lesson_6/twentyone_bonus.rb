...
def increment_score(score, player_hand, dealer_hand)
  if hand_value(player_hand) == hand_value(dealer_hand)
    ...
    score[determine_winner(player_hand, dealer_hand)[0].to_sym] += 1
  end
end
...
def display_game_winner(scores)
  if score_check(scores)
    prompt("The #{scores.key(ROUNDS_TO_WIN).to_s} has won #{ROUNDS_TO_WIN} rounds!")
    prompt("The #{scores.key(ROUNDS_TO_WIN).to_s} wins the game!")
    true
  end
end
...
loop do
  ...
  increment_score(scores, player_hand, dealer_hand)
