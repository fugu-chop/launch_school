...
class TextTest < MiniTest::Test
  ...
  def teardown(output: $stdout)
    output = StringIO.new
    @file.close
    # Normally, this would provide a visual check of whether file is closed (). 
    # However, since we use the StringIO.puts, the text is consumed and nothing shows in the terminal when running out tests
    output.puts "File has been closed: #{@file.closed?}"
  end
end
